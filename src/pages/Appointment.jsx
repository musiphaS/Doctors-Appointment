    ...
    setDocSlots([])
      ...
      for(let i=0 ; i<7; i++) {
        ...
        if(today.getDate() === currentDate.getDate()) {
          currentDate.setHours(currentDate > 10 ? currentDate.getHours() + 1:10)
  ...
  return docInfo &&  (
    <div>
      ...
      <div className="sm:ml-72 sm:pl-4 mt-4 font-medium text-gray-700">
        <p>Booking Slots</p>
        <div className="flex gap-3 item-center overflow-x-scroll w-full mt-4 scrollbar-hide">
          {
            docSlots.length && docSlots.map((item,index)=> (
                <div onClick={() => setSlotIndex(index)} className={`text-center py-6 min-w-16 rounded-full cursor-pointer ${slotIndex === index ? 'bg-blue-300 text-white' : 'border border-gray'}`} key={index}>
        ...
        <button className="bg-blue-300 text-white text-sm font-light px-14 py-3 rounded-full my-6">Book an appointment</button>
      </div>
